...
class TestDataGenerator {
    ...
    void generateTestData() {
        ...
        if (seatSectionsExist== null || !seatSectionsExist) {
            ...
            String[] grades = {"NORMAL", "VIP", "NORMAL"};
        ...
        if (eventsExist== null || !eventsExist) {
            System.out.println("=== Step 3: Event 생성 (50만개) ===");
            ...
            String[] statuses = {"OPEN", "SCHEDULED", "CLOSED"};
        ...
        if (sessionsExist== null || !sessionsExist) {
            System.out.println("=== Step 4: EventSession 생성 (50만개) ===");
            ...
            String sessionSql = "INSERT INTO event_sessions (event_id, session, session_seat_count, status, session_open_date, session_close_date, created_at, updated_at) " +
                    "SELECT e.id, '1회차', 500, 'SCHEDULED', e.open_date, e.end_date, ?, ? " +
                    "FROM events e " +
        ...
        // 5. TicketType (Session당 평균 2개씩 = 100만개)
        ...
        if (ticketTypesExist== null || !ticketTypesExist) {
            System.out.println("=== Step 5: TicketType 생성 (100만개) ===");

            String ticketSql = "INSERT INTO ticket_types " +
                    ...
                    "    CASE " +
                    "        WHEN ss.grade = 'VIP' THEN 80000 + (es.id % 20000) " +
                    "        ELSE 50000 + (es.id % 20000) " +
                    "    END AS price, " +
                    "    'ON_SALE' AS ticket_type_status, " +
